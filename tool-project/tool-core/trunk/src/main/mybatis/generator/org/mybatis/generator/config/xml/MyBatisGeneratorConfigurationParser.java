...
public class MyBatisGeneratorConfigurationParser {
    ...
    private void parseClassPathEntry(Configuration configuration, Node node) {
        ...
        if(null == location || location.isEmpty()) {        	
        	// 修改原来获取数据库驱动jar包路径  lixx
        	configuration.addClasspathEntry(this.getClass().getClassLoader().getResource("mysql-connector-java-5.1.29.jar").getPath());
        } else {
        	configuration.addClasspathEntry(location); //$NON-NLS-1$
        }
    }
