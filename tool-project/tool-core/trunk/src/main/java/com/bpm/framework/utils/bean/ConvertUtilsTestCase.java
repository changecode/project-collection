 ...
 * @version $Revision: 1.2 $ $Date: 2010/10/26 08:54:52 $
 */
...
public class ConvertUtilsTestCase extends TestCase {
	...
	public void testNegativeIntegerArray() {
		...
		value = ConvertUtils.convertGt((String) null, intArray.getClass());
	...
	/**
	 * Negative scalar conversion tests. These rely on the standard default
	 * value conversions in ConvertUtils.
	 */
	public void testNegativeScalar() {
		...
		booleanValue = ConvertUtils.convertGt("foo", Boolean.TYPE);
		// assertTrue(booleanValue instanceof Boolean);
		assertEquals(booleanValue.booleanValue(), false);
	...
	public void testObjectToStringScalar() {
		...
		assertEquals("Boolean->String", "false", ConvertUtils
				.convert(Boolean.FALSE));
		assertEquals("Boolean->String", "true", ConvertUtils
				.convert(Boolean.TRUE));
		assertEquals("Byte->String", "123", ConvertUtils.convert((byte) 123));
		assertEquals("Character->String", "a", ConvertUtils.convert('a'));
		assertEquals("Double->String", "123.0", ConvertUtils.convert(123.0d));
		assertEquals("Float->String", "123.0", ConvertUtils.convert(123.0f));
		assertEquals("Integer->String", "123", ConvertUtils.convert(123));
		assertEquals("Long->String", "123", ConvertUtils.convert(123L));
		assertEquals("Short->String", "123", ConvertUtils.convert((short) 123));
	...
	public void testPositiveStringArray() {
		...
		checkStringArray(value, stringArray2);
		value = ConvertUtils
				.convert("\"abc\",\"de,f\"", stringArray.getClass());
		checkStringArray(value, stringArray2);
		value = ConvertUtils
				.convert("{\"abc\" 'de,f'}", stringArray.getClass());
		checkStringArray(value, stringArray2);
		value = ConvertUtils.convert("'abc' 'de,f'", stringArray.getClass());
		checkStringArray(value, stringArray2);
		value = ConvertUtils.convert("{'abc', \"de,f\"}", stringArray
				.getClass());
		checkStringArray(value, stringArray2);
	...
	public void testDeregisteringSingleConverter() throws Exception {
		...
		Object value = ConvertUtils.convert("true", Boolean.TYPE);
		assertTrue(value instanceof Boolean);
		assertEquals("Standard conversion failed (1)", ((Boolean) value)
				.booleanValue(), true);

		// we'll test deregister
		ConvertUtils.deregister(Boolean.TYPE);
		assertNull("Converter should be null", ConvertUtils
				.lookup(Boolean.TYPE));
	...
	private void checkIntegerArray(Object value, int intArray[]) {
		...
		assertEquals("Returned value is int[]", intArray.getClass(), value
				.getClass());
	...
	private void checkStringArray(Object value, String stringArray[]) {
		...
		assertNotNull("Returned value is not null", value);
		assertEquals("Returned value is String[]", stringArray.getClass(),
				value.getClass());
		String results[] = (String[]) value;
		assertEquals("Returned array length", stringArray.length,
				results.length);
		for (int i = 0; i < stringArray.length; i++) {
			assertEquals("Returned array value " + i, stringArray[i],
					results[i]);
		}
