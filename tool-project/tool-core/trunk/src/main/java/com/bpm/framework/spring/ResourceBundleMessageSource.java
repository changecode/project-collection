package com.bpm.framework.spring;
...
public class ResourceBundleMessageSource extends
	...
	private static final Logger log = Logger.getLogger(ResourceBundleMessageSource.class);
	 ...
	 */
	private final Map<String, ResourceBundleMessageSource.PropertiesHolder> cachedProperties = new HashMap<String, ResourceBundleMessageSource.PropertiesHolder>();
	/**
	 * Cache to hold merged loaded properties per locale
	 */
	private final Map<Locale, ResourceBundleMessageSource.PropertiesHolder> cachedMergedProperties = new HashMap<Locale, ResourceBundleMessageSource.PropertiesHolder>();
	...
	protected String resolveCodeWithoutArguments(String code, Locale locale) {
		if (this.cacheMillis < 0) {
			ResourceBundleMessageSource.PropertiesHolder propHolder = getMergedProperties(locale);
			...
			for (String basename : this.basenames) {
				...
				for (String filename : filenames) {
					ResourceBundleMessageSource.PropertiesHolder propHolder = getProperties(filename);
					String result = propHolder.getProperty(code);
					if (result != null) {
						return result;
					}
	 ...
	 */
	protected ResourceBundleMessageSource.PropertiesHolder getMergedProperties(
			Locale locale) {
		synchronized (this.cachedMergedProperties) {
			ResourceBundleMessageSource.PropertiesHolder mergedHolder = this.cachedMergedProperties
					.get(locale);
			if (mergedHolder != null) {
				return mergedHolder;
			}
			Properties mergedProps = new Properties();
			mergedHolder = new ResourceBundleMessageSource.PropertiesHolder(
					mergedProps, -1);
	 ...
	 */
	protected ResourceBundleMessageSource.PropertiesHolder getProperties(
			String filename) {
		synchronized (this.cachedProperties) {
			ResourceBundleMessageSource.PropertiesHolder propHolder = this.cachedProperties
					.get(filename);
			if (propHolder != null
	 ...
	 */
	protected ResourceBundleMessageSource.PropertiesHolder refreshProperties(
			String filename,
			ResourceBundleMessageSource.PropertiesHolder propHolder) {
		...
		try {
			...
			if (resources != null && resources.length > 0) {
				propHolder = new ResourceBundleMessageSource.PropertiesHolder();
				for (Resource resource : resources) {
					if (resource.exists()) {
						...
						try {
							...
							if (log.isDebugEnabled()) {
								log.debug("Could not parse properties file ["
						...
						if (log.isDebugEnabled()) {
							log.debug("No properties file found for ["
				...
				// Resource does not exist.
				if (log.isDebugEnabled()) {
					log.debug("No properties files found for [" + filename
							+ "] - neither plain properties nor XML");
				}
			...
			if (log.isDebugEnabled()) {
				log.debug("Could not match pattern [" + filename + "]",
						iOException);
			}
	...
	public void clearCacheIncludingAncestors() {
		clearCache();
		if (getParentMessageSource() instanceof ResourceBundleMessageSource) {
			((ResourceBundleMessageSource) getParentMessageSource())
