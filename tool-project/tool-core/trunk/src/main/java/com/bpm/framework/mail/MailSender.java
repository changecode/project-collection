...
public class MailSender implements Serializable {
	 ...
	 * @param targetAddr
	 */
	public static void sendSysMail(String title,String content,List<String> targetAddr){
		try{
			// pop.exmail.qq.com
			Properties props = new Properties();
			props.put("mail.smtp.auth", "true");// 默认就是true
			// smtp.exmail.qq.com  pop.exmail.qq.com 都可以
			MailSender s = new MailSender("smtp.exmail.qq.com", "sys@sunline.cn", "123456", props);
	...
	public static void sendInfoMail(String title,String content,List<String> targetAddr){
		try{
			...
			String emailServer = EmailPropCache.getInstance().getMap().get("email.server");
			String emailUser = EmailPropCache.getInstance().getMap().get("email.user");
			String emailPwd = EmailPropCache.getInstance().getMap().get("email.pwd");
			String emailFrom = EmailPropCache.getInstance().getMap().get("email.from");
			// pop.exmail.qq.com
			Properties props = new Properties();
			props.put("mail.smtp.auth", "true");// 默认就是true
			// smtp.exmail.qq.com  pop.exmail.qq.com 都可以
	...
	public static void main(String... args) {
		...
		targets.add("123000226@qq.com");
		MailSender.sendInfoMail(mailTitle, content.toString(), targets);
	}
}
