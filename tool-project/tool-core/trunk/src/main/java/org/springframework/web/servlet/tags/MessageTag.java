 ...
 * @author andyLee  为了解决界面label不经过controller就需要进行国际化处理，重写spring自带的MessageTag.java
 * @see #setCode
 * @see #setText
 * @see #setHtmlEscape
...
public class MessageTag extends HtmlEscapingAwareTag implements ArgumentAware {
	...
	protected String resolveMessage() throws JspException, NoSuchMessageException {
		...
		// begin lixx 处理<spring:message />标签不经过controller的情况下根据session数据进行国际化 2015-12-01 15:18:00
		Locale locale = getRequestContext().getLocale();// 默认值
		...
		// end lixx 处理<spring:message />标签不经过controller的情况下根据session数据进行国际化 2015-12-01 15:18:00
		
		// Evaluate the specified MessageSourceResolvable, if any.
		if (this.message != null) {
			// We have a given MessageSourceResolvable.
			return messageSource.getMessage(this.message, locale);
		}
		...
		if (this.code != null || this.text != null) {
			...
			if (this.text != null) {
				...
				return messageSource.getMessage(
						this.code, argumentsArray, this.text, locale);
			}
			else {
				// We have no fallback text to consider.
				return messageSource.getMessage(
						this.code, argumentsArray, locale);
			}
		}
